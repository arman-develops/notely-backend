...
export async function createUser(req: Request, res: Response) {
  try {
    ...
    bcrypt.hash(password, saltRounds, async (err, hash) => {
      if (err) {
        sendErrorResponse(
          res,
          { err },
          "Oops soemthing broke. Try again later",
        );
    ...
    sendErrorResponse(
      res,
      { error },
      "Error registering your profile. Try again",
    );
    console.log(error);
  }
}
...
export async function userOnboarding(req: Auth, res: Response) {
  try {
    const userID = req.user?.userID;
    const { bio, avatar, preferences, hasCompletedOnboarding, firstNote } =
      req.body;
